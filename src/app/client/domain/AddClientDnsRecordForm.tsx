...
import { addClientDnsRecord } from "@/server/client-domain";
...
export function AddClientDnsRecordForm({ domain }: { domain: string }) {
  const router = useRouter();
  const [type, setType] = useState<string>("A");
  const [state, action, pending] = useActionState(addClientDnsRecord, { ok: false } as Result);
    ...
    <form action={action} className="card space-y-4 p-6">
      ...
      <div>
        ...
        <p className="mt-1 text-xs text-navy-400">
          ...
          {type === "TXT" && "Free-form text (SPF, domain verification, …)."}
        </p>
      </div>
